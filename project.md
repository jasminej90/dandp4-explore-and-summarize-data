...
univ_cont <- function(feature, bins) {
  ggplot(aes_string(x = feature), data = rw) +
    geom_histogram(color = "black", fill = "#993366", binwidth = bins)
}
...
```{r , Univariate_plot_vacid}

univ_cont("volatile.acidity", 0.05)
...
```{r , Univariate_plot_cacid}

univ_cont("citric.acid", 0.03)
...
```{r, Univariate_plot_rsugar}

# p1 <- univ_cont("residual.sugar", 0.25)

p0 <- ggplot(aes(x = residual.sugar), data = rw)

p1 <- p0 + geom_histogram(color = "black", fill = "#993366", binwidth = 0.25)

p2 <- p0 + geom_histogram(color = "black", fill = "#993366", binwidth = 0.05) + scale_x_log10()

p3 <- p0 + geom_histogram(color = "black", fill = "#993366", binwidth = 0.02) + scale_x_sqrt()
...
```{r, Univariate_plot_chloride}

p1 <- univ_cont("chlorides", 0.025)
...
```{r, Univariate_plot_fsd}

p1 <- univ_cont("free.sulfur.dioxide", 5)

p2 <- univ_cont("free.sulfur.dioxide", 0.1) +
  scale_x_log10()

p3 <- univ_cont("total.sulfur.dioxide", 10)

p4 <- univ_cont("total.sulfur.dioxide", 0.05) +
  scale_x_log10()
...
```{r, Univariate_plot_alcoh}

univ_cont("alcohol", 0.25)
...
rw$grade <-
  cut(rw$quality,breaks = c(2.5,4.5,6.5,8.5),
      labels = c('OK','Good','V.Good'))

# Turn alcohol percentages into a string categorical factor
rw$alcohol.intensity <-
  cut(rw$alcohol, breaks = c(7.9, 10.4, 12.9, 15.4),
      labels = c('Low.Alcohol','Medium.Alcohol','High.Alcohol'))

ggplot(aes(x = grade), data = rw) +
  geom_bar(color = "black", fill = "#993366")

ggplot(aes(x = alcohol.intensity), data = rw) +
  geom_bar(color = "black", fill = "#993366")

```

Here, I plot a barvchart of two newly created variables describing wine grade and alcohol intensity.
...
```{r, Bivariate_Plots_Alcho_and_Qual}
...
The original plot of wine quality and alcohol looks overplotted, so I decided to plot another one with transparency, jitter effects and boxplot. I also graphed the average, 1st, 2nd, and 3rd quantile. As alcohol increases, wine quality increases too. Most of good-quality (rating of 5.5 and higher) wine is of alcohol concentration between 9.5 - 13%.
...
```{r, Bivariate_Plots_Sulph_and_Citric}
...
```{r echo=FALSE, Bivariate_Plots_Boxp3}
...
ggplot(aes(x = citric.acid, y = density), data = rw) +
  geom_point(aes(color = alcohol.intensity),
             position = position_jitter(h = 0), alpha = 0.6) +
  geom_smooth(aes(color = alcohol.intensity), se = F) +
  scale_colour_brewer(palette = "YlOrRd")  + theme_dark()
...
ggplot(aes(x = density, y = volatile.acidity), data = rw) +
  ...
  facet_grid(alcohol.intensity ~ .) + theme(legend.position="none")
...
### Description Two
Wine quality correlates positively with alcohol concentration with correlation coefficient of 0.48. From the plot, 75% of my wine dataset has an alcohol concentration less than 13%.

### Plot Three
```{r, Plot_Three}

p1 <- ggplot(aes(x = alcohol, y = volatile.acidity), data = rw) +
  geom_point(aes(color = grade), position = position_jitter(h = 0)) +
  scale_colour_brewer(palette = "RdPu")  + theme_dark() +
...
p2 <- ggplot(aes(x = citric.acid, y = chlorides), data = rw) +
  geom_point(aes(color = grade), position = position_jitter(h = 0), alpha = 0.6) +
  geom_line(stat = 'summary', fun.y = mean, color = 'white') +
  scale_colour_brewer(palette = "RdPu")  + theme_dark() +
