...
p2 <- univ_cont("sulphates", 0.07) + scale_x_log10(breaks = c(0.5, 0.75, 1))
...
ggplot(aes(x = as.factor(quality) ,y = volatile.acidity), data = rw) +
  geom_point(position = position_jitter(h = 0)) +
  geom_boxplot(alpha = 0.5) +
  stat_summary(fun.y = "median", colour = "red", size = 2, geom = "point")
...
ggplot(aes(x = pH, y = residual.sugar, color = grade), data = rw) +
  geom_point(alpha = 0.4) +
  geom_smooth(method = 'lm', se = F) +
  coord_cartesian(xlim = c(2.9, 3.75)) +
  scale_colour_brewer(palette = "RdPu")  + theme_dark() 

ggplot(aes(x = sulphates, y = volatile.acidity, color = grade), data = rw) +
  geom_point(alpha = 0.4) +
  geom_smooth(method = 'lm', se = F) +
  scale_colour_brewer(palette = "RdPu") +
  theme_dark()

ggplot(aes(x = citric.acid, y = chlorides, color = grade), data = rw) +
  geom_point(position = position_jitter(h = 0), alpha = 0.6) +
  geom_smooth(method = 'lm', se = F) +
  scale_colour_brewer(palette = "RdPu")  + theme_dark()
...
p2 <- ggplot(aes(x = citric.acid, y = chlorides), data = rw) +
  ...
  geom_smooth(se = F, aes(color = grade)) +
  scale_colour_brewer(palette = "RdPu")  + theme_dark() +
